...
func getUNCReservation(_ userId:String, resultCallback: @escaping (_ alertMessage:String)->Void ) -> Void {
    ...
    let task = URLSession.shared.dataTask(with: loginPage) {
        ...
        if error == nil && data != nil {
            ...
            if let path = params["path"], let token = params["token"] {
                ...
                doAction(userId, path, token, boundary) {
                    (pag, res, err) in
                    if err == nil && pag != nil {
                ...
                resultCallback("Error en el parseo de login")
            }
        } else {
            resultCallback("Error en la conexion")
        }
    }
    task.resume()
}

private func doAction(_ userId:String,_ path:String,_ token:String,_ boundary:String, login:Bool=true, callback: @escaping (_ data:Data?, _ response:URLResponse?, _ error:Error?)->Void){
...
private func parseWeb(page:String!, getAlert:Bool=false) -> [String:String] {
    ...
    if getAlert {
        ...
        if let alerts = page.findall(pattern: "UD REGISTRA.*;</script></div>"){
            for alert in alerts {
